<?php include('config.php'); ?>
	...
	<div class="container" style="margin-top:20px">
		...
		if(isset($_GET['id'])){
			...
			$select = mysqli_query($koneksi, "SELECT * FROM kepengurusan WHERE id='$id'") or die(mysqli_error($koneksi));
		...
		if(isset($_POST['submit'])){
			...
			$pindah = move_uploaded_file($tmpgambar,"./assets/upload_gambar/kepengurusan/".$namafile);
			if($pindah){
			@unlink("./assets/upload_gambar/kepengurusan/".$gambar_lama);

			$sql = mysqli_query($koneksi, "UPDATE kepengurusan SET nama='$nama', jabatan='$jabatan', keterangan='$keterangan', gambar='$namafile' WHERE id='$id'") or die(mysqli_error($koneksi));

			if($sql){
				echo '<script>alert("Berhasil menyimpan data."); document.location="dashbord.php?page=tampil_kepengurusan";</script>';
			}else{
		...
		<form action="editkepengurusan.php?id=<?php echo $id; ?>" method="post" enctype="multipart/form-data">
			...
			<div class="item form-group">
				...
				<img style="width: 120px;" src="./assets/upload_gambar/kepengurusan/<?php echo $data['gambar'];?>">
				<div class="col-md-6 col-sm-6">
			...
			<div class="item form-group">
				<div class="col-md-6 col-sm-6 offset-md-3">
					...
					<a href="dashbord.php?page=tampil_kepengurusan" class="btn btn-warning">Kembali</a>
				</div>
			</div>
		</form>
	</div>
